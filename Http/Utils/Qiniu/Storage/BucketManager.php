...
*                                            ]
    ...
    public function listFiles(
        ...
        \Modules\AntOA\Http\Utils\Qiniu\Utils::setWithoutEmpty($query, 'prefix', $prefix);
        \Modules\AntOA\Http\Utils\Qiniu\Utils::setWithoutEmpty($query, 'marker', $marker);
        \Modules\AntOA\Http\Utils\Qiniu\Utils::setWithoutEmpty($query, 'limit', $limit);
        \Modules\AntOA\Http\Utils\Qiniu\Utils::setWithoutEmpty($query, 'delimiter', $delimiter);
...
*                                            ]
    ...
    public function listFilesv2(
        ...
        \Modules\AntOA\Http\Utils\Qiniu\Utils::setWithoutEmpty($query, 'limit', $limit);
        \Modules\AntOA\Http\Utils\Qiniu\Utils::setWithoutEmpty($query, 'delimiter', $delimiter);
        \Modules\AntOA\Http\Utils\Qiniu\Utils::setWithoutEmpty($query, 'skipconfirm', $skipconfirm);
...
*                                              ]
    ...
    {
        $path = '/stat/' . \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($bucket, $key);
    ...
    {
        $path = '/delete/' . \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($bucket, $key);
    ...
    {
        $from = \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($from_bucket, $from_key);
        $to = \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        if ($force === true) {
            $path .= '/force/true';
        }
    ...
    {
        $resource = \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($bucket, $key);
        $encode_mime = \Modules\AntOA\Http\Utils\Qiniu\Utils::base64_urlSafeEncode($mime);
    ...
    public function fetch($url, $bucket, $key = null)
    {
        ...
        $resource = \Modules\AntOA\Http\Utils\Qiniu\Utils::base64_urlSafeEncode($url);
        $to = \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($bucket, $key);
    ...
    {
        $entry = \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($bucket, $key);
    ...
    public static function buildBatchDeleteAfterDays($bucket, $key_day_pairs)
    {
        ...
        foreach ($key_day_pairs as $key => $day) {
            array_push($data, '/deleteAfterDays/' . \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($bucket, $key) . '/' . $day);
        }
        return $data;
    }
    ...
    public static function buildBatchChangeMime($bucket, $key_mime_pairs)
    {
        ...
        foreach ($key_mime_pairs as $key => $mime) {
            array_push($data, '/chgm/' . \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($bucket, $key) . '/mime/' . base64_encode($mime));
        }
        return $data;
    }
    ...
    public static function buildBatchChangeType($bucket, $key_type_pairs)
    {
        ...
        foreach ($key_type_pairs as $key => $type) {
            array_push($data, '/chtype/' . \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($bucket, $key) . '/type/' . $type);
        }
        return $data;
    }
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($bucket, $key));
        }
        return $data;
    }
    ...
    private static function twoKeyBatch($operation, $source_bucket, $key_pairs, $target_bucket, $force)
    {
        ...
        foreach ($key_pairs as $from_key => $to_key) {
            $from = \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($source_bucket, $from_key);
            $to = \Modules\AntOA\Http\Utils\Qiniu\Utils::entry($target_bucket, $to_key);
