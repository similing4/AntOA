...
final class ResumeUploader
{
    ...
    public function __construct(
        ...
        list($accessKey, $bucket, $err) = \Modules\AntOA\Http\Utils\Qiniu\Utils::explodeUpToken($upToken);
        if ($err != null) {
    ...
    public function upload($fname)
    {
        ...
        while ($uploaded < $this->size) {
            ...
            $crc = \Modules\AntOA\Http\Utils\Qiniu\Utils::crc32_data($data);
            ...
            if ($response->statusCode < 0) {
                list($accessKey, $bucket, $err) = \Modules\AntOA\Http\Utils\Qiniu\Utils::explodeUpToken($this->upToken);
                if ($err != null) {
                    return array(null, $err);
                }
    ...
    private function fileUrl($fname)
    {
        ...
        $url .= '/mimeType/' . \Modules\AntOA\Http\Utils\Qiniu\Utils::base64_urlSafeEncode($this->mime);
        if ($this->key != null) {
            $url .= '/key/' . \Modules\AntOA\Http\Utils\Qiniu\Utils::base64_urlSafeEncode($this->key);
        }
        $url .= '/fname/' . \Modules\AntOA\Http\Utils\Qiniu\Utils::base64_urlSafeEncode($fname);
        if (!empty($this->params)) {
            foreach ($this->params as $key => $value) {
                $val = \Modules\AntOA\Http\Utils\Qiniu\Utils::base64_urlSafeEncode($value);
                $url .= "/$key/$val";
            }
        }
        return $url;
