<template>
	<div>
		<span>{{tip}}</span>
		<a-button type="primary" @click="isShow = true;$forceUpdate()">选择</a-button>
		<a-modal v-model="isShow" :title="'请选择' + column.tip">
			...
			<standard-table :columns="columns" v-if="columns.length > 0" :data-source="dataSource"
				:pagination="pagination" :selected-rows.sync="selectedRows" :row-key="columns[0].dataIndex"
				...
				<div slot="action" slot-scope="{text, record}">
					...
					<a-button @click="select(record)" type="primary" class="linebtn">选择
					</a-button>
				</div>
			</standard-table>
		</a-modal>
	...
	export default {
		...
		methods: {
			async loadPage() {
				...
				let res = await this.$api(this.api.detail_column_list + "?type=" + this.pagetype + "&col=" + this
						.column.col)
