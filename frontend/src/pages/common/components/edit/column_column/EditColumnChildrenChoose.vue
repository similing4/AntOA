    ...
    <a-form-item :label="column.tip" :label-col="{span: 7}" :wrapper-col="{span: 10}">
        ...
        <a-modal v-model="isSelectModalShow" :title="'请选择' + column.tip">
            <div style="margin-bottom: 16px">
                <a-row>
                    <a-col :md="12" :sm="24" v-for="(filterItem,indexT) in column.gridListEasy.listFilterCollection" :key="indexT">
                    ...
                    <span style="float: right; margin-top: 3px;" v-if="column.gridListEasy.listFilterCollection.filter((t)=>t.type != 'ListFilterHidden').length > 0">
            ...
            <a-space class="antoa-list-operator">
                <EasyHeaderButtonsWrapper :grid-list-object="column.gridListEasy" @openurl="openurl" @loadpage="loadPage" />
            </a-space>
            <standard-table :columns="tableModel.columns" v-if="tableModel.columns.length > 0" :data-source="tableModel.dataSource" :selected-rows.sync="tableModel.selectedRows" :pagination="tableModel.pagination" :row-key="column.gridListVModelCol" @change="onDataChange">
                <template :slot="templateItem.col" slot-scope="{text, record}" v-for="templateItem in column.gridListEasy.listTableColumnCollection">
                ...
                <div slot="action" slot-scope="{text, record}">
                    <EasyRowButtonsWrapper :grid-list-object="column.gridListEasy" :record="record" @openurl="openurl" @loadpage="loadPage" @select="select(record)" />
                </div>
            </standard-table>
        </a-modal>
        <slot />
