		...
		<a-form v-if="api != null">
			<template v-for="(column,index) in columns">
				<a-form-item :label="column.tip" :label-col="{span: 7}" :wrapper-col="{span: 10}"
					...
					<a-button v-if="getApiButtonByColumn(column.col)" :type="getApiButtonByColumn(column.col).type"
				...
				<a-form-item :label="column.tip" :label-col="{span: 7}" :wrapper-col="{span: 10}"
					...
					<a-input-number :placeholder="'请填写' + column.tip" v-model="form[column.col]"></a-input-number>
					{{column.extra.unit}}
					<a-button v-if="getApiButtonByColumn(column.col)" :type="getApiButtonByColumn(column.col).type"
						@click="callApi(getApiButtonByColumn(column.col).url)">
						@{{getApiButtonByColumn(column.col).title}}
				...
				<a-form-item :label="column.tip" :label-col="{span: 7}" :wrapper-col="{span: 10}"
					...
					<a-tree-select v-model="form[column.col]" :tree-data="column.extra" style="width: 100%"
						tree-checkable :search-placeholder="'请选择' + column.tip"></a-tree-select>
					<a-button v-if="getApiButtonByColumn(column.col)" :type="getApiButtonByColumn(column.col).type"
						@click="callApi(getApiButtonByColumn(column.col).url)">
						{{getApiButtonByColumn(column.col).title}}
					</a-button>
						...
						for (let i in this.columns) {
							if (res.data[this.columns[i].col] !== undefined) {
								if (this.columns[i].type === 'COLUMN_CHECKBOX' || this.columns[i].type ===
									'COLUMN_PICTURES' || this.columns[i].type === 'COLUMN_FILES' || this.columns[i]
									.type === 'COLUMN_CHOOSE' || this.columns[i].type === 'COLUMN_TREE_CHECKBOX')
			...
			reset() {
				for (let i in this.columns) {
					...
					;
						...
						this.columns[i].type === 'COLUMN_FILES' || this.columns[i].type === 'COLUMN_CHOOSE' || this
						.columns[i].type === 'COLUMN_TREE_CHECKBOX')
