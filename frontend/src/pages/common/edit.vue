			...
			<template v-for="(column,index) in columns">
				...
				<a-form-item :label="column.tip" :label-col="{span: 7}" :wrapper-col="{span: 10}"
					...
					<column-children-choose v-model="form[column.col]" :tip.sync="formTip[column.col]"
						v-if="formTip[column.col]" :column="column" :api="api"></column-children-choose>
				</a-form-item>
			</template>
					...
					if (res.status) {
						for (let i in this.columns) {
							...
							if (this.columns[i].type === 'COLUMN_CHILDREN_CHOOSE')
								this.formTip[this.columns[i].col] = res.tip[this.columns[i].col][this.columns[
									i].extra.displayColumn];
						}
					} else
						throw res.msg;
				} catch (e) {
