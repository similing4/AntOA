    ...
    <div class="content">
      ...
      <div class="login">
        <a-form @submit="onSubmit" :form="form">
          <a-form-item>
            <a-input autocomplete="autocomplete" name="username" size="large" :placeholder="'请输入' + username_placeholder" v-model="username">
          ...
          <a-form-item>
            <a-input
              size="large"
              name="password"
              :placeholder="'请输入' + password_placeholder"
          ...
          <a-form-item>
            <a-button :loading="logging" class="submitbtn" style="width: 100%; margin-top: 24px" size="large" html-type="submit" type="primary"
              >登录
            </a-button>
          </a-form-item>
        </a-form>
...
export default {
  ...
  async created() {
    try {
      if (!this.$api().checkAuthorization()) throw '登录失败';
      const res = await this.$api('/api/antoa/auth/auth').method('POST').call();
      if (!res.status) throw '登录失效';
      this.goHome();
    } catch (e) {
      console.log(e);
    }
  ...
  mounted() {
    try {
      ...
      if (!usernameAndPassword) throw '没有发现本地存储的用户名密码';
