		...
		return ((function() {
						...
						encode: function(str) {
							return (str + '').replace(/./g, function(v) {
								...
								for (var i = 0; i < bitArr.length; i++) {
									bitArr[i] = '%' + ('0' + bitArr[i].toString(16))
										.substr(
											-2)
										.toUpperCase();
								}
			...
			var Fn_unzip = function unZip() {
				return arguments[0].replace(/\x23(\d+)\x24/g, function(a, b) {
					...
					.replace(/\x23/g, "###")
					.replace(/([\x26-\x7e]{2})\x21([\x26-\x7e\x23]+)(?:\x20|$)/g, function(all, hd,
						dt) {
			...
			var GBK = function() {
				...
				// 以GBk编码顺数排列的编码映射数组
				var data_sort_gbk = getSortGBK(Fn_unzip(
				...
				var data_sort_unicode = data_sort_gbk.slice(0).sort(function(a, b) {
					return a[0] - b[0];
				});
