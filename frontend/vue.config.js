...
module.exports = {
	...
	configureWebpack: config => {
    	...
    	fs.writeFileSync(ListTableColumnPluginGeneralFile, `<template>` + ListTableColumnPluginGeneralFileComponentsList.map((t)=>`<` + t.name + ` v-if="render.type == '` + t.name + `'" :item="item" :value="value" @input="$emit('input',$event)" />`).join("") + `</template><script>` + ListTableColumnPluginGeneralFileComponentsList.map((t)=>`import ` + t.name + ` from "` + t.path + `";`).join("") + `export default{props:["render","value"],data(){return {};},components:{` + ListTableColumnPluginGeneralFileComponentsList.map((t)=> t.name + `,`).join("") + `}}</script>`);
